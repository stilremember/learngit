define(['jquery','deskIndex','md5','myLib','layer','customScrollbar'],function($,deskIndex, md5, myLib, layer, cust){
    var serviceList={
        ...
        MeauAppService:"/awater/desktop/json/MeauApp.json",
        listByLoginName:"/awater/desktop/json/listByLoginName.json"
    ...
    function getAppUrl() {
        var urlRegexp = /^(?:[A-Za-z]+:)?\/{0,3}(?:[0-9.\-A-Za-z]+)(?::\d+)?(?:\/[^/?#]*)/,
    ...
    function init(){
        ...
        $.ajax({
            url: serviceList.MeauAppService,
            type:"get",
            dataType:"json",
            async: false,
...
//弹出子系统
    ...
    function initDeskIcon() {
        $.ajax({
            url: serviceList.listByLoginName,
            type:"get",
            data: {
                ...
                for(var i=0; i<systemList.length; i++) {
                    ...
                    if(sysItem.isMustShowInDesktop === '1' && sysItem.isSetVisibleRange != '1' ) {//&& !sysItem.isUserBinded
                        ...
                        if(sysItem.position=='left'){
                            liHtmlContent += ''
                                + '<li class="desktop_icon" url="'+sysItem.appUrl+'"' +
    ...
    function initByCount(count){
        if(count>=1){
            deskIndex.desktopPanel();
            deskIndex.initDeskIcon();
        }
    }
